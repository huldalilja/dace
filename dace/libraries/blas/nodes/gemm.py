...
class ExpandGemmTensorCore2(ExpandTransformation):
    ...
    def expansion(node, state, sdfg):
        ...
        host_code_io_stream.write("""DACE_EXPORTED void calling_gemm(dace::float16* a, dace::float16* b, float* c);
        """)
...
#pragma unroll
    ...
    const half *warp_ptr = (warpId < 4) ? (&A[block_tile_i * WMMA_M * K_GLOBAL] +
                                           WMMA_M * K_GLOBAL * (warpId % 4) * 2)
                                        : (&B[block_tile_j * WMMA_N * K_GLOBAL] +
                                           WMMA_N * K_GLOBAL * (warpId % 4) * 2);
...
DACE_EXPORTED void calling_gemm(dace::float16* a, dace::float16* b, float* c);
void calling_gemm(dace::float16* a, dace::float16* b, float* c){
...
for(int l = 0; l < {WMMA_M}*{WMMA_N}; l++){{
    cfrag.x[l] = {alpha} * accfrag.x[l]{beta_code};
        ...
        # Adding local storage (shared memory)
        # smem_a = InLocalStorage.apply_to(ksdfg, dict(array='_a'), node_a=aslice, node_b=atile)
        # smem_b = InLocalStorage.apply_to(ksdfg, dict(array='_b'), node_a=bslice, node_b=btile)
...
class Gemm(dace.sdfg.nodes.LibraryNode):
    ...
    implementations = {
        ...
        "TensorCore": ExpandGemmTensorCore2,
